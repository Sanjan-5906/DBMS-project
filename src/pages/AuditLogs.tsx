  ...
  return (
    <div className="space-y-8 animate-fade-up">
      <section className="overflow-hidden rounded-[2rem] border border-border/80 bg-gradient-to-r from-white via-yellow-50 to-white p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            <Badge variant="outline" className="w-fit rounded-full px-4 py-1.5 border-yellow-300">System Monitoring</Badge>
      ...
      <Card className="border-border/80 bg-white/95 shadow-sm">
        ...
        <CardContent>
          {isLoading ? (
            <div className="py-20 text-center"><Loader2 className="mx-auto animate-spin text-amber-500" /></div>
          ) : (
            <div className="space-y-4">
              {filtered.map((log) => (
                <div key={log.log_id} className="group relative flex flex-col gap-4 rounded-2xl border border-border p-5 transition-all hover:border-yellow-200 hover:bg-yellow-50/30">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        "flex h-10 w-10 items-center justify-center rounded-xl shadow-sm",
                        log.action === 'INSERT' ? "bg-emerald-100 text-emerald-700" :
                        log.action === 'UPDATE' ? "bg-yellow-100 text-amber-700" :
