...
const columns: ReadOnlyColumn<Transaction>[] = [
  ...
  { key: "transaction_type", label: "Transaction Type", render: (transaction) => <span className="capitalize">{transaction.transaction_type}</span> },
  {
    key: "amount",
    label: "Amount",
    render: (transaction) =>
...
export default function Transactions() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Transaction ID", "Account ID", "Type", "Amount", "Date"];
    const csvContent = [
      headers.join(","),
      ...displayedTransactions.map(tx => `"${tx.transaction_id}","${tx.account_id}","${tx.transaction_type}","${tx.amount}","${tx.transaction_date}"`)
    ].join("\n");
