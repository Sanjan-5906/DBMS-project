import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import {
  ArrowDown,
  ArrowUp,
  ArrowUpDown,
  Edit3,
  Landmark,
  PlusCircle,
  Search,
  Trash2,
  Wallet,
  Download
} from "lucide-react";
import type { Account, AccountSortKey, SortDirection } from "./account-types";

type AccountTableProps = {
  accounts: Account[];
  totalRecords: number;
  totalPages: number;
  currentPage: number;
  searchQuery: string;
  sortKey: AccountSortKey;
  sortDirection: SortDirection;
  isLoading: boolean;
  onSearchChange: (value: string) => void;
  onSortChange: (key: AccountSortKey) => void;
  onEditAccount: (account: Account) => void;
  onRequestDelete: (account: Account) => void;
  onPageChange: (page: number) => void;
  onClearSearch: () => void;
  onAddAccount: () => void;
};

const sortableColumns: Array<{ key: AccountSortKey; label: string }> = [
  { key: "account_id", label: "Account ID" },
  { key: "customer_id", label: "Customer ID" },
  { key: "branch_id", label: "Branch ID" },
  { key: "account_type", label: "Account Type" },
  { key: "balance", label: "Balance" },
];

function formatCurrency(balance: number) {
  return new Intl.NumberFormat("en-IN", {
    style: "currency",
    currency: "INR",
    maximumFractionDigits: 2,
  }).format(balance);
}

export default function AccountTable({
  accounts,
  totalRecords,
  totalPages,
  currentPage,
  searchQuery,
  sortKey,
  sortDirection,
  isLoading,
  onSearchChange,
  onSortChange,
  onEditAccount,
  onRequestDelete,
  onPageChange,
  onClearSearch,
  onAddAccount,
}: AccountTableProps) {

  const handleExportCSV = () => {
    if (accounts.length === 0) return;
    const headers = ["Account ID", "Customer ID", "Branch ID", "Account Type", "Balance"];
    const csvContent = [
      headers.join(","),
      ...accounts.map(a => `"${a.account_id}","${a.customer_id}","${a.branch_id}","${a.account_type}","${a.balance}"`)
    ].join("\n");
    
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", "accounts_export.csv");
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  if (isLoading) {
    return (
      <Card className="border-slate-200 bg-white shadow-sm dark:bg-slate-900/95">
        <CardHeader>
          <div className="space-y-3">
            <div className="h-6 w-48 animate-pulse rounded-full bg-slate-100 dark:bg-slate-800" />
            <div className="h-4 w-64 animate-pulse rounded-full bg-slate-100 dark:bg-slate-800" />
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="h-11 w-full animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-800" />
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className="h-16 animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-800" />
            ))}
          </div>
        </CardContent>
      </Card>
    );
  }

  const hasRecords = accounts.length > 0;
  const showNoResults = totalRecords === 0;

  return (
    <Card className="border-slate-200 bg-white shadow-sm dark:bg-slate-900/95">
      <CardHeader className="space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            <Badge variant="outline" className="w-fit rounded-full px-3 py-1 bg-blue-50 text-blue-800 border-blue-200">
              Account Directory
            </Badge>
            <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-slate-100">
              <Wallet className="h-5 w-5 text-blue-800" />
              Account Records Table
            </CardTitle>
            <CardDescription className="text-slate-600 dark:text-slate-300">Search, sort, page through, and manage account records.</CardDescription>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
            <div className="flex items-center gap-2 font-semibold text-slate-900 dark:text-slate-100">
              <Landmark className="h-4 w-4 text-blue-800" />
              {totalRecords} {totalRecords === 1 ? "record" : "records"}
            </div>
            <p className="mt-1">Page {currentPage} of {totalPages}</p>
          </div>
        </div>

        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full lg:max-w-xl">
            <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 dark:text-slate-300" />
            <input
              type="search"
              value={searchQuery}
              onChange={(event) => onSearchChange(event.target.value)}
              placeholder="Search by account, customer, branch, type, or balance"
              className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-24 text-sm text-slate-900 outline-none transition focus:border-blue-800 focus:bg-white focus:ring-2 focus:ring-blue-800/20"
            />
            {searchQuery ? (
              <button
                type="button"
                onClick={onClearSearch}
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-white hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-slate-100"
              >
                Clear
              </button>
            ) : null}
          </div>

          <div className="flex items-center gap-2">
            <Button variant="outline" className="rounded-xl border-slate-200 text-slate-700 hover:bg-slate-50 lg:w-fit" onClick={handleExportCSV} disabled={!hasRecords}>
              <Download className="h-4 w-4 mr-2" />
              Export CSV
            </Button>
            <Button className="rounded-xl bg-blue-800 hover:bg-blue-900 text-white shadow-md shadow-blue-900/20 lg:w-fit" onClick={onAddAccount}>
              <PlusCircle className="h-4 w-4 mr-2" />
              Add Account
            </Button>
          </div>
        </div>
      </CardHeader>

      <CardContent className="space-y-5">
        {showNoResults ? (
          <div className="rounded-[1.75rem] border border-dashed border-blue-200 bg-blue-50/70 p-8 text-center dark:border-slate-700 dark:bg-slate-900">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-white text-blue-800 shadow-sm dark:bg-slate-800 dark:text-blue-300">
              <Search className="h-7 w-7" />
            </div>
            <h3 className="mt-4 text-xl font-bold text-slate-900 dark:text-slate-100">
              {searchQuery ? "No matching accounts found" : "No account records available"}
            </h3>
            <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-300">
              {searchQuery
                ? "Try a different search term or clear the filter to view all stored records."
                : "Use the Add Account button to create the first local account record."}
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              {searchQuery ? (
                <Button variant="outline" className="rounded-xl" onClick={onClearSearch}>
                  Clear Search
                </Button>
              ) : null}
              <Button className="rounded-xl bg-blue-800 hover:bg-blue-900 text-white" onClick={onAddAccount}>
                <PlusCircle className="h-4 w-4 mr-2" />
                Add Account
              </Button>
            </div>
          </div>
        ) : (
          <div className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                <thead className="bg-slate-50 text-xs uppercase tracking-[0.18em] text-slate-500 dark:bg-slate-800 dark:text-slate-300">
                  <tr>
                    {sortableColumns.map((column) => {
                      const active = sortKey === column.key;

                      return (
                        <th key={column.key} scope="col" className="px-5 py-4 font-semibold">
                          <button
                            type="button"
                            onClick={() => onSortChange(column.key)}
                            className="inline-flex items-center gap-2 transition hover:text-slate-900 dark:hover:text-slate-100"
                            aria-sort={active ? (sortDirection === "asc" ? "ascending" : "descending") : "none"}
                          >
                            {column.label}
                            {active ? (
                              sortDirection === "asc" ? (
                                <ArrowUp className="h-3.5 w-3.5" />
                              ) : (
                                <ArrowDown className="h-3.5 w-3.5" />
                              )
                            ) : (
                                <ArrowUpDown className="h-3.5 w-3.5 text-slate-400 dark:text-slate-300" />
                            )}
                          </button>
                        </th>
                      );
                    })}
                    <th scope="col" className="px-5 py-4 font-semibold">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 bg-white dark:bg-slate-900">
                  {hasRecords ? (
                    accounts.map((account) => (
                      <tr key={account.account_id} className="group transition-colors hover:bg-blue-50/60">
                        <td className="px-5 py-4">
                          <Badge variant="outline" className="rounded-full border-blue-200 bg-white text-slate-900 dark:border-blue-900 dark:bg-slate-800 dark:text-slate-100">
                            {account.account_id}
                          </Badge>
                        </td>
                        <td className="px-5 py-4 font-semibold text-slate-900 dark:text-slate-100">{account.customer_id}</td>
                        <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{account.branch_id}</td>
                        <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{account.account_type}</td>
                        <td className="px-5 py-4 font-semibold text-slate-900 dark:text-slate-100">{formatCurrency(account.balance)}</td>
                        <td className="px-5 py-4">
                          <div className="flex flex-wrap gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              className="rounded-lg border-slate-200 bg-white text-slate-700 hover:border-blue-300 hover:bg-blue-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700"
                              onClick={() => onEditAccount(account)}
                            >
                              <Edit3 className="h-3.5 w-3.5 mr-2" />
                              Edit
                            </Button>
                            <Button
                              variant="outline"
                              size="sm"
                              className="rounded-lg border-rose-200 bg-white text-rose-600 hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700 dark:border-rose-900 dark:bg-slate-800 dark:text-rose-300 dark:hover:bg-rose-950/60"
                              onClick={() => onRequestDelete(account)}
                            >
                              <Trash2 className="h-3.5 w-3.5 mr-2" />
                              Delete
                            </Button>
                          </div>
                        </td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={6} className="px-5 py-14 text-center text-sm text-slate-500 dark:text-slate-300">
                        No records are available for the current view.
                      </td>
                    </tr>
                  )}
                </tbody>
              </table>
            </div>
          </div>
        )}

        {totalPages > 1 && hasRecords ? (
          <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700 dark:bg-slate-800">
            <p className="text-sm text-slate-600 dark:text-slate-300">
              Showing {(currentPage - 1) * 5 + 1} to {Math.min(currentPage * 5, totalRecords)} of {totalRecords} records
            </p>
            <div className="flex flex-wrap items-center gap-2">
              <Button
                variant="outline"
                size="sm"
                className="rounded-lg border-slate-200 text-slate-700 hover:bg-slate-100"
                onClick={() => onPageChange(Math.max(1, currentPage - 1))}
                disabled={currentPage === 1}
              >
                Previous
              </Button>
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
                <Button
                  key={page}
                  variant={page === currentPage ? "default" : "outline"}
                  size="sm"
                  className={cn("min-w-10 rounded-lg px-3", page === currentPage ? "bg-blue-800 hover:bg-blue-900 text-white" : "border-slate-200 text-slate-700 hover:bg-slate-100 bg-white dark:bg-slate-900")}
                  onClick={() => onPageChange(page)}
                >
                  {page}
                </Button>
              ))}
              <Button
                variant="outline"
                size="sm"
                className="rounded-lg border-slate-200 text-slate-700 hover:bg-slate-100"
                onClick={() => onPageChange(Math.min(totalPages, currentPage + 1))}
                disabled={currentPage === totalPages}
              >
                Next
              </Button>
            </div>
          </div>
        ) : null}
      </CardContent>
    </Card>
  );
}
